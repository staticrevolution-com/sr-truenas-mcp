...
| B4 | CI: SBOM + Sigstore artifact attestation | ✅ done | `release.yml` permissions gain `id-token: write` + `attestations: write`. Binary job: `anchore/sbom-action` → SPDX SBOM, `actions/attest-build-provenance` signs the tarball, `actions/attest-sbom` links them; release upload includes the `.spdx.json`. Docker job: captures pushed-image digest, `actions/attest-build-provenance push-to-registry: true` writes the attestation alongside the image. All actions pinned to commit SHA per `portainer-safety.md` / supply-chain hygiene. Verification post-release: `gh attestation verify <binary> --owner staticrevolution-com` and `gh attestation verify oci://<image>@<digest> --owner staticrevolution-com`. |
