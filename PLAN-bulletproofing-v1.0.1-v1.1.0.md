...
| B2 | UUID request IDs + opt-in keepalive | ✅ done | `crypto.randomUUID()` for all DDP request ids (was `String(++this.requestId)`); existing test for incrementing-string ids updated to assert UUID shape. New `keepaliveIntervalMs` field on `TrueNASClientConfig` + `ServerConfig`; CLI reads `TRUENAS_KEEPALIVE_INTERVAL_MS` (default 0/disabled). Keepalive timer is `unref`-ed and cleaned up on cleanup(); 3 new tests cover off/on/post-close. |
...
| B4 | CI: SBOM + Sigstore artifact attestation | ⏭ partial / deferred | **SBOM lands** (`anchore/sbom-action` → `sr-truenas-mcp-linux-x64.spdx.json` uploaded to release alongside tarball + sha256). **Attestations deferred** — `actions/attest-build-provenance` and `actions/attest-sbom` require either a paid GitHub plan or a public repo, neither of which currently apply to `staticrevolution-com`. Steps removed from `release.yml`; comment block in the workflow notes how to re-add them when the repo flips public. v1.1.0 release-attempt #1 failed on this; re-cut without attestations succeeded. |
...
- **MCP SDK 2.0 migration** when SDK 2.0 stabilizes. Today: `2.0.0-alpha.2` (April 2026); stable expected Q1 2026 per the [v2 docs](https://ts.sdk.modelcontextprotocol.io/v2/). Headline change is *Standard Schema* — SDK 2.x is **schema-agnostic** via the [`~standard`](https://standardschema.dev) interop, so existing Zod schemas pass through with a thin wrapper, no schema rewrite. Our actual SDK call surface is tiny (1 × `new McpServer`, 1 × `server.tool` for the hierarchical `truenas` tool, 12 × `server.resource`, 1 × `StdioServerTransport`); the 277 action handlers go through our `ToolRegistry` shim and don't touch the SDK. ~15 lines change. v1.x will get bug-fix + security backports for ≥6 months after v2 ships, so no rush. Forcing function: stable v2 release. **Prep landed**: `src/mcp-adapter.ts` is now the only runtime importer of `@modelcontextprotocol/sdk` symbols; every other file uses `import type` for `McpServer`. The eventual SDK-2.0 migration is a one-file diff plus a single import path change (10 type-only sites).
- **Zod v4 migration** — 6.5× faster, but `.passthrough()/.strict()/.strip()` semantics change (we hit one `.passthrough()` site at `src/registry.ts:218`). Originally listed as paired with SDK 2.0; **that pairing was wrong** — SDK 2.x is schema-agnostic, so Zod v4 is fully decoupled and can move on its own schedule. Treat as an independent perf-only minor. Forcing function: measured validation cost showing up in latency budgets.
